...
def test_cart_shows_multiple_items(cart_page_with_multiple_items):
    """Verify that multiple items are shown in the cart."""
...
def test_remove_one_item_from_cart(cart_page_with_multiple_items):
    """Verify that removing one item from the cart works as expected."""
...
def test_cart_prices_match_inventory(cart_page_with_multiple_items):
    ...
    # Capture inventory prices while genuinely on /inventory.html.
    inventory.navigate()
    inventory_prices = {item: inventory.get_item_price(item) for item in items}

    # Return to the cart and assert each cart price against the snapshot.
    cart.navigate()
    for item in items:
